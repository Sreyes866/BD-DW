    ...
    <div className="container my-5">
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          ...
          <select id="templateType" name="templateType" value={article.templateType} onChange={handleChange}>
        ...
        <div className="form-group">
          ...
          <select id="category_id" name="category_id" value={article.category_id} onChange={handleChange}>
            <option value="" disabled>Seleccione una categoría</option>
            {Object.keys(categories).map((category_id, index) => (
              <option key={index} value={category_id}>{category_id}</option>
            ))}
          </select>
        </div>
        <div className="form-group">
          <label htmlFor="sub_category_id">Subcategoría</label>
          <select id="sub_category_id" name="sub_category_id" value={article.sub_category_id} onChange={handleChange}>
            <option value="" disabled>Seleccione una subcategoría</option>
            {categories[article.category_id]?.map((sub_category_id, index) => (
              <option key={index} value={sub_category_id}>{sub_category_id}</option>
            ))}
          </select>
        </div>
        <div className="form-group">
          <label htmlFor="author_id">Autor</label>
          <input type="text" id="author_id" name="author_id" value={article.author_id} onChange={handleChange} />
        </div>
        <button type="submit">Crear</button>
      </form>

      {/* Añadir esta parte para mostrar un mensaje cuando el artículo se crea con éxito */}
      {articleCreated && <div className="alert alert-success" role="alert">
        Artículo creado con éxito. Puedes verlo <a href={articleLink}>aquí</a>.
      </div>}
