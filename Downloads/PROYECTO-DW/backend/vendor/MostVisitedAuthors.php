...
$sql = "SELECT author, total_visits FROM MostVisitedAuthors ORDER BY total_visits DESC LIMIT 5";
...
if ($stmt = $conn->prepare($sql)) {
    ...
    if ($stmt->execute()) {
        ...
        while ($row = $result->fetch_assoc()) {
            $mostVisitedAuthors[] = [
                'author' => $row['author'], // Asumiendo que 'author' es el nombre del autor
